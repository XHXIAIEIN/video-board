...
function buildProject({ title, uploader, duration, width, height, enCues, zhCues }) {
  ...
  const chapId = (t) =>
    "ch" + Math.min(nChap, Math.floor(t / CHAP_SECS) + 1);
  const chapters = [];
  ...
  const zhFor = (s, e) => {
    const mid = (s + e) / 2;
    const z = zhCues.find((z) => mid >= z.start && mid < z.end) ||
              zhCues.find((z) => z.end > s && z.start < e);
    return z ? z.text : "";
  };
...
function splitTracks(subTexts) {
  const ranked = subTexts
    .map((t) => ({ ...t, r: cjkRatio(t.text) }))
    .sort((a, b) => a.r - b.r); // least Chinese first
...
(function wireLanding() {
  ...
  const fail = (msg) => { err.textContent = msg; drop.classList.remove("busy"); };
  ...
  async function ingest(files) {
    ...
    try {
      const subTexts = await Promise.all(
        subs.map(async (f) => ({ name: f.name, text: await f.text() })),
      );
  ...
  document.getElementById("load-demo").addEventListener("click", async () => {
    ...
    try {
      ...
      fail("示例加载失败（需经 http 访问，本地请用 serve.py 启动）：" + (ex && ex.message ? ex.message : ex));
    } finally {
      drop.classList.remove("busy");
    }
  });
